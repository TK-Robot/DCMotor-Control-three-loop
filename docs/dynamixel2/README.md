...
- 波特率代码：`2=115200`、`3=1 Mbps`、`4=2 Mbps`；单播 ACK 发送完成后运行期切换，写 NVM 后才跨复位保留。
...
- 电流保护分三级：地址 `336` 为滤波后非锁存峰值削波，地址 `338` 为滤波后持续绝对过流锁存，地址 `340/342/344` 为速度/位置模式的延时堵转保护。单个未滤波 ADC 尖峰仅保留为诊断证据，不直接锁存故障。
